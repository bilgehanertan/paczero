...
class PACTrainer(Trainer):
    ...
    def _inner_training_loop(
        ...
        # trajectory replays consistently. HF Trainer normally calls this in its
        # _inner_training_loop; our override skipped it. Privacy bound holds either
        # way (cum_mi gate enforces ≤ pac_mi), but reproducibility/forensic-replay
        # of released bits across resume now matches.
        ...
        #   uniform  (default): beta_t = MI_total / T for all t. Simple, but if many
        ...
        for epoch in range(epochs_trained, max_steps):
            ...
            for step, inputs in enumerate(epoch_iterator):
                ...
                # ============================================================
                # PACZero-kvar: K-direction PAC step (DP-AggZO style, K≥1)
                # ------------------------------------------------------------
                # ============================================================

                # 1. Pop membership vectors (shared across K directions)
                ...
                # 2. Collect trainable parameters (shared across K)
                self.named_parameters_to_optim = []
                for name, param in model.named_parameters():
                    if param.requires_grad:
                        self.named_parameters_to_optim.append((name, param))

                # K-loop accumulators (DP-AggZO style averaging across K directions)
                k_random_seeds = []
                k_released_bits = []
                k_mi_step_sum = 0.0  # Σ over K releases this step
                k_unanimous_true_count = 0  # # of K iterations with full unanimity
                k_unanimous_pac_count = 0  # # of K iterations with PAC unanimous
                k_release_types = []
                k_sigma_useds = []
                k_signal_raws = []
                k_var_subset_raws = []
                ...
                k_batch_means = []
                k_s_secrets = []
                k_per_step_mi_capped_any = False

                # Per-step total MI budget (uniform or adaptive — computed ONCE per step;
                # adaptive uses cum_mi which is unchanged within a step). Split equally
                # across K releases. K=1 reduces to per_step_mi == per_step_mi_total.
                if pac_adaptive_mi:
                    _remaining_steps = max(max_steps - self.state.global_step, 1)
                    per_step_mi_total = max(pac_mi - cum_mi, 0.0) / _remaining_steps
                else:
                    per_step_mi_total = per_step_mi_uniform
                per_step_mi_per_release = per_step_mi_total / max(pac_k, 1)

                # ----- K-direction loop (DP-AggZO style, K independent ZO estimates) -----
                for _k_iter in range(pac_k):
                    # 3.k Sample direction k, compute per-sample directional derivatives
                    ...
                    # 4.k Per-sample clipping (stability; doesn't affect sign)
                    sample_abs = np.abs(ghat_np)
                    pre_clip_mean = float(sample_abs.mean())
                    pre_clip_max = float(sample_abs.max())
                    clip_scale = np.minimum(
                        1.0, pac_clip / np.maximum(sample_abs, 1e-12)
                    )
                    ghat_clipped = ghat_np * clip_scale
                    clip_fired_frac = float((sample_abs > pac_clip).mean())

                    # 5.k Compute M subset means, then sign-quantize
                    m_ghats = compute_subset_ghats(ghat_clipped, membership_np, pac_m)
                    signs = np.where(m_ghats >= 0, 1.0, -1.0).astype(np.float64)

                    # 6.k Per-direction diagnostic stats
                    ...
                    agreement_frac = max(q_plus, 1.0 - q_plus)
                    ...
                    # Per-release MI budget = per_step_mi_per_release (= per_step_mi_total/K).
                    # H(S)-cap applied per release — preserves PAC chain-rule admissibility.
                    per_step_mi = per_step_mi_per_release
                    per_step_mi_capped = False
                    if (q_plus > 0.0) and (q_plus < 1.0):
                        h_s_binary = float(
                            -q_plus * np.log(q_plus)
                            - (1.0 - q_plus) * np.log(1.0 - q_plus)
                        )
                        h_s_cap = 0.999 * h_s_binary
                        if per_step_mi > h_s_cap:
                            per_step_mi = h_s_cap
                            per_step_mi_capped = True
                            k_per_step_mi_capped_any = True

                    pos_sign_count = int((signs > 0).sum())
                    unanimous_true = (pos_sign_count == 0) or (pos_sign_count == pac_m)
                    if unanimous_true:
                        k_unanimous_true_count += 1
                    if unanimous:
                        k_unanimous_pac_count += 1

                    release_type = None

                    # 7.k PAC mechanism: sign-quantized release (UNCHANGED from v4 K=1)
                    if no_privacy:
                        ...
                        # PRIVATE MODE (PAC-MI with Gaussian noise)
                        if unanimous:
                            released_bit = s_secret
                            sigma_used = 0.0
                            mi_step = 0.0
                    ...
                    # Append per-direction state to K-loop accumulators
                    k_random_seeds.append(random_seed)
                    k_released_bits.append(float(released_bit))
                    k_mi_step_sum += float(mi_step)
                    k_release_types.append(release_type)
                    k_sigma_useds.append(float(sigma_used))
                    k_signal_raws.append(signal_raw)
                    k_var_subset_raws.append(var_subset_raw)
                    k_q_pluses.append(q_plus)
                    k_pos_sign_counts.append(pos_sign_count)
                    k_pre_clip_means.append(pre_clip_mean)
                    k_pre_clip_maxes.append(pre_clip_max)
                    k_clip_fired_fracs.append(clip_fired_frac)
                    k_batch_means.append(batch_mean_scalar)
                    k_s_secrets.append(s_secret)
                # ----- end of K-direction loop -----

                # Update cumulative MI (single += per training step; K releases summed)
                cum_mi += k_mi_step_sum
                if k_mi_step_sum > 0.0:
                    n_noisy_steps += 1
                else:
                    n_free_steps += 1

                # Aggregate K-direction stats for diagnostic logging.
                # Re-bind variable names that downstream diag logger uses.
                # (Means across K — sums for budget-related fields.)
                signal_raw = float(np.mean(k_signal_raws)) if k_signal_raws else 0.0
                batch_mean_scalar = (
                    float(np.mean(k_batch_means)) if k_batch_means else 0.0
                )
                var_subset_raw = (
                    float(np.mean(k_var_subset_raws)) if k_var_subset_raws else 0.0
                )
                q_plus = float(np.mean(k_q_pluses)) if k_q_pluses else 0.0
                agreement_frac = max(q_plus, 1.0 - q_plus)
                unanimous = k_unanimous_pac_count == pac_k  # all-K unanimous
                sigma_used = float(np.mean(k_sigma_useds)) if k_sigma_useds else 0.0
                mi_step = float(k_mi_step_sum)
                per_step_mi = float(per_step_mi_total)  # the per-step total budget
                per_step_mi_capped = bool(k_per_step_mi_capped_any)
                # release_type aggregate: most common across K (fall back to last)
                if k_release_types:
                    from collections import Counter as _C

                    release_type = _C(k_release_types).most_common(1)[0][0]
                else:
                    release_type = None
                unanimous_true = k_unanimous_true_count == pac_k
                pos_sign_count = (
                    float(np.mean(k_pos_sign_counts)) if k_pos_sign_counts else 0.0
                )
                s_secret = float(np.mean(k_s_secrets)) if k_s_secrets else 0.0
                projected_grad = (
                    float(np.mean(k_released_bits)) if k_released_bits else 0.0
                )  # ∈ [-1, 1]
                released_bit = projected_grad
                pre_clip_mean = (
                    float(np.mean(k_pre_clip_means)) if k_pre_clip_means else 0.0
                )
                pre_clip_max = (
                    float(np.mean(k_pre_clip_maxes)) if k_pre_clip_maxes else 0.0
                )
                clip_fired_frac = (
                    float(np.mean(k_clip_fired_fracs)) if k_clip_fired_fracs else 0.0
                )

                # SNR (per-step, aggregate)
                snr_per_step = signal_raw / (sigma_used if sigma_used > 0 else 1e-12)

                # 8. Update parameters: θ_new = θ - lr · (1/K) Σ_k bit_k · z_k(seed_k)

                lr = self._get_learning_rate()
                _inv_k = 1.0 / max(pac_k, 1)

                # Pre-allocate per-param accumulators (one full param-set worth of memory).
                update_buffers = {}
                for name, param in self.named_parameters_to_optim:
                    update_buffers[name] = torch.zeros_like(param.data)

                # K-loop: seed ONCE per direction, then iterate params in same order
                # as perturbation phase. This reproduces the per-direction z sequence
                # exactly, including the K=1 case (which then matches original v4).
                for _seed_k, _bit_k in zip(k_random_seeds, k_released_bits):
                    torch.manual_seed(_seed_k)
                    _coef_k = _bit_k * _inv_k
                    for name, param in self.named_parameters_to_optim:
                        z_k = torch.normal(
                            mean=0,
                            std=1,
                        ...
                        update_buffers[name] = update_buffers[name] + _coef_k * z_k
                        del z_k

                # Apply accumulated update (with weight decay where applicable)
                for name, param in self.named_parameters_to_optim:
                    update_buf = update_buffers[name]
                    if (
                        "bias" not in name
                        ...
                        param.data = param.data - lr * (
                            update_buf + args.weight_decay * param.data
                        )
                    else:
                        param.data = param.data - lr * update_buf
                update_buffers.clear()
                ...
                diag_entry = {
                    ...
                    "pac_m": pac_m,
                    "pac_k": int(pac_k),
                    "k_mi_step_sum": float(k_mi_step_sum),
                    "k_unanimous_true_count": int(k_unanimous_true_count),
                    "k_unanimous_pac_count": int(k_unanimous_pac_count),
                    "k_release_types": k_release_types,
        ...
        # Dev-plateau model selection: reload the best-dev checkpoint before the
        # caller runs the final test eval.
