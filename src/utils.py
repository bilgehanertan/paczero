...
def forward_wrap_with_option_len_dpzero(self, input_ids=None, labels=None, option_len=None, num_options=None, return_dict=None, dpzero=False, **kwargs):
    ...
    if num_options is not None: 
        ...
        if any([x != num_options[0] for x in num_options]):
            ...
            if dpzero:
                # Per-sample losses for PAC privacy
                per_sample_losses = []
                start_id = 0
                while start_id < len(num_options):
                    ...
                    _loss = loss_fct(_logits, _labels)  # (1,) with reduction='none'
                    per_sample_losses.append(_loss)
                    start_id = end_id
                loss = torch.cat(per_sample_losses)  # (bsz,)
            else:
                loss = 0
                start_id = 0
                count = 0
...
class PACDataCollator:
    """
    Wrapper collator that strips 'membership_vector' from features before
    passing to the inner collator, then re-attaches them to the batch.

    Handles both:
    - Dict features (only_train_option mode): each feature is a dict
    - List-of-dict features (train_as_classification mode): each feature is a
      list of dicts (one per candidate). Each candidate dict in a sample shares
      the same membership_vector. We extract one per original sample.
    """
    ...
    def __call__(self, features):
        ...
        for f in features:
            if isinstance(f, dict):
                f_copy = {k: v for k, v in f.items() if k != 'membership_vector'}
                if 'membership_vector' in f:
                    membership_vectors.append(f['membership_vector'])
                clean_features.append(f_copy)
            elif isinstance(f, list):
                # train_as_classification: list of dicts per sample
                # Extract membership_vector from the first candidate (all share the same one)
                inner_list = []
                for i, d in enumerate(f):
                    d_copy = {k: v for k, v in d.items() if k != 'membership_vector'}
                    if i == 0 and 'membership_vector' in d:
                        membership_vectors.append(d['membership_vector'])
                    inner_list.append(d_copy)
                clean_features.append(inner_list)
            else:
                clean_features.append(f)

        batch = self.inner_collator(clean_features)

        if membership_vectors:
            batch['membership_vector'] = torch.stack(membership_vectors)
